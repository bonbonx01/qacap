...
from models.clip_qavit import InstructCLIPVisionModel
...
class CLIPVisionTower(nn.Module):
    def __init__(self, config, instruction_dim):
        ...
        self.integration_point = config['integration_point'] if 'integration_point' in config.keys() else None
        self.load_model()

    def load_model(self):
        self.image_processor = CLIPImageProcessor.from_pretrained(self.vision_tower_name) # 图像预处理器 将img -> tensor
        if self.clip_type == 'qavit':
            ...
            self.vision_tower = InstructCLIPVisionModel(config=config, instruction_dim=self.instruction_dim,
                                                        integration_point=self.integration_point) #关键
