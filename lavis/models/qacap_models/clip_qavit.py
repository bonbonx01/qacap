...
class MMCLIPAttention(CLIPAttention):
    ...
    def forward(
        self,
        hidden_states: torch.Tensor, # 图像特征
        attention_mask: Optional[torch.Tensor] = None,
        causal_attention_mask: Optional[torch.Tensor] = None,
        output_attentions: Optional[bool] = False,
        kv_states: torch.Tensor = None, # 文本指令特征 
        kv_masks: torch.Tensor = None, 
    ) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        """Input shape: Batch x Time x Channel"""

        mm_len = kv_states.shape[1] # 获得文本指令长度
        hidden_states = torch.cat([kv_states, hidden_states], dim=1) # 文本指令特征 和 图像特征 在序列维度上拼接起来
        bsz, tgt_len, embed_dim = hidden_states.size() 

        # get query proj 计算 Q K V
        ...
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2)) # 计算注意力权重
        ...
        # 关键操作：只保留由图像部分产生的query 所计算出的注意力分数 
...
class MMCLIPEncoderLayer(nn.Module):
    def __init__(self, config: CLIPConfig):
        ...
        self.self_attn = MMCLIPAttention(config) # 关键 注意力机制的实现 
...
class InstructCLIPEncoder(nn.Module):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        modules_list = []
        for layer_id in range(config.num_hidden_layers): #控制 融合时机 config.num_hidden_layers 取决于 vit-model 的选取
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                if isinstance(encoder_layer, CLIPEncoderLayer): #如果是标准的CLIP层，只传入视觉相关的参数
                ...
                else: # 如果是定制的MMCLIPEncoderLayer层，则额外传入文本指令信息
                    layer_outputs = encoder_layer( 
                        hidden_states,
                        attention_mask,
                        causal_attention_mask,
            ...
            # hidden_states 中存储的就是经过所有层处理后的、最终的、问题感知的视觉特征。

            if output_attentions: # 如果需要，保存当前层的注意力权重
                all_attentions = all_attentions + (layer_outputs[1],)

        if output_hidden_states:
            encoder_states = encoder_states + (hidden_states,)
            #encoder_states 是每一层的输出 
        if not return_dict:
        ...
        # hidden_states: 每一层的【隐藏状态】输出，形状为 (num_layers, batch_size, sequence_length, hidden_size)
        # attentions: 每一层的注意力权重，形状为 (num_layers, batch_size, num_heads, sequence_length, sequence_length)
...
class CLIPVisionTransformer(nn.Module):
    def __init__(self, config: CLIPVisionConfig):
        ...
        self.encoder = InstructCLIPEncoder(config) # 关键 
    ...
    def forward(
        ...
        # --- 步骤 1: 图像嵌入 (Embeddings) ---
        # 将输入的图像像素转换为图像块嵌入 + 位置嵌入
        hidden_states = self.embeddings(pixel_values)
        hidden_states = self.pre_layrnorm(hidden_states)
        
        # --- 步骤 3: 核心编码过程 (Encoder) ---
        # 将嵌入向量和文本指令一起送入 InstructCLIPEncoder
        ...
        # --- 步骤 5: 特征池化 (Pooling) 与后归一化 ---
        # 提取序列的第一个token（通常是 [CLS] token）的特征作为整个图像的全局表示
